...
# --- DimRed page ---
    ...
    color_by = st.selectbox("Color by", ["Primary Type","kmeans_label"], index=0 if "Primary Type" in dr_df.columns else 1)
    if color_by in dr_df.columns:
        chart = alt.Chart(dr_df).mark_circle(size=8).encode(
            x="dim1", y="dim2", color=color_by, tooltip=["Primary Type","kmeans_label","PC1"]
...
# --- Monitoring page ---
    ...
    tracking_uri = st.sidebar.text_input("MLflow tracking URI", "http://localhost:5008")
    ...
    if exp is None:
        ...
        if st.button("Open MLflow UI (localhost port 5000)"):
            st.write("Open your MLflow UI at http://localhost:5008")
